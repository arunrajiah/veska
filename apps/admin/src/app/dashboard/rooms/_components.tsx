  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          d.error ??
            (d.conflict ? 'This slot conflicts with an existing booking.' : 'Failed to book room'),
        );
        return;
      }
      onBooked(d);
          ...
          {rooms.map((room) => (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize flex-shrink-0 ${STATUS_BADGE[room.status ?? 'available'] ?? STATUS_BADGE.available}`}
                >
              ...
              {room.amenities && room.amenities.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {room.amenities.map((a) => (
                    <span
                      key={a}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 text-xs capitalize"
                    >
                      {AMENITY_ICONS[a] ?? ''} {a}
                    </span>
                  ))}
...
function BookRoomTab({ rooms }: { rooms: Room[] }) {
  ...
  const [bookingFor, setBookingFor] = useState<{ room: Room; start: string; end: string } | null>(
    null,
  );
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        {rooms.map((room) => {
          ...
          return (
            <div
              key={room.id}
              className="flex items-center border-b border-gray-50 last:border-0 px-4 py-3 gap-4"
            >
              <div className="w-40 flex-shrink-0">
  ...
  return (
    <div className="px-8 py-8 max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage meeting rooms and book spaces for your team.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <>
          {tab === 'rooms' && (
            <RoomsTab rooms={rooms} summary={summary} onAddRoom={() => setShowAddRoom(true)} />
          )}
