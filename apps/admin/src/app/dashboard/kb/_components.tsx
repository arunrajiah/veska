    ...
    <div>
      ...
      >
        ...
        {cat.articleCount != null && (
          <span
            className={`text-[10px] px-1.5 py-0.5 rounded-full flex-shrink-0 ${isSelected ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-500'}`}
          >
            {cat.articleCount}
          </span>
        )}
  ...
  return (
    <div className="w-[400px] border-l border-gray-200 bg-white flex flex-col h-full flex-shrink-0">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 flex-shrink-0">
        <h2 className="font-semibold text-gray-900 text-sm line-clamp-1 flex-1 pr-3">
          {d.title ?? '—'}
        </h2>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400">
          {d.status && (
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[d.status] ?? 'bg-gray-100 text-gray-600'}`}
            >
              {d.status}
            </span>
          )}
        ...
        <div className="flex items-center gap-4 text-xs text-gray-400">
          <span className="flex items-center gap-1">
            <Eye size={11} /> {d.views ?? 0} views
          </span>
          <span className="flex items-center gap-1">
            <ThumbsUp size={11} className="text-green-500" /> {d.helpful ?? 0}
          </span>
          <span className="flex items-center gap-1">
            <ThumbsDown size={11} className="text-red-400" /> {d.notHelpful ?? 0}
          </span>
        </div>
        ...
        {(d.tags ?? []).length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            {(d.tags ?? []).map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-0.5 text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full"
              >
                <Tag size={9} /> {tag}
              </span>
            ))}
...
export function KBPageClient({
  ...
  const publishedArticles =
    summary.publishedArticles ?? articles.filter((a) => a.data.status === 'published').length;
  ...
  const filtered = articles.filter((a) => {
    if (
      selectedCategory &&
      a.data.categoryId !== selectedCategory &&
      a.data.category !== selectedCategory
    ) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[220px] border-r border-gray-200 bg-white flex flex-col flex-shrink-0 overflow-y-auto py-3 px-2">
          ...
          >
            ...
            All Articles
            <span
              className={`ml-auto text-[10px] px-1.5 py-0.5 rounded-full ${selectedCategory === null ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-500'}`}
            >
              {articles.length}
            </span>
          </button>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-medium text-gray-900 line-clamp-1 flex-1 text-sm">
                        {d.title ?? '—'}
                      </h3>
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${STATUS_COLORS[d.status ?? 'draft'] ?? 'bg-gray-100 text-gray-600'}`}
                      >
                        {d.status ?? 'draft'}
                      </span>
                    </div>
