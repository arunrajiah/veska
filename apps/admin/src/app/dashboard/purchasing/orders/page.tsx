...
export default async function PurchaseOrdersPage() {
  ...
  try {
    const res = await apiFetch<OrderRecord[]>('/api/v1/purchasing/orders', tenantId);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">PO Number</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Expected date</th>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      <Link
                        href={`/dashboard/purchasing/orders/${record.id}`}
                        className="font-mono text-xs text-gray-600 hover:text-gray-900"
                      >
                        {poNumber}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-gray-600">{String(d['vendor_id'] ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{String(d['order_date'] ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{String(d['expected_date'] ?? '—')}</td>
