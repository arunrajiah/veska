...
export function AllTasksClient({
  ...
  const filtered = filterStatus ? tasks.filter((t) => (t.data?.status ?? 'todo') === filterStatus) : tasks;
    ...
    <div className="px-8 py-8 max-w-7xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{total} task{total !== 1 ? 's' : ''}</p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((task) => {
                ...
                const projectName = d?.projectId ? (projectMap[d.projectId] ?? d.projectId.slice(0, 8) + '…') : '—';
                ...
                return (
                  <tr key={task.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                    ...
                    <td className="px-5 py-3 text-xs text-gray-500">
                      {d?.projectId ? (
                        <a href={`/dashboard/projects/${d.projectId}`} className="hover:text-gray-900">{projectName}</a>
                      ) : '—'}
                    </td>
                    <td className="px-5 py-3 text-gray-500">{d?.assigneeName ?? '—'}</td>
                    <td className="px-5 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${PRIORITY_STYLES[priority] ?? 'bg-gray-100 text-gray-500'}`}>
                        {priority}
                      </span>
                    </td>
                    <td className="px-5 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${STATUS_STYLES[status] ?? 'bg-gray-100 text-gray-600'}`}>
                        {status.replace(/_/g, ' ')}
                      </span>
                    </td>
