      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-50">
          {enrollments.map((en) => {
            const d = en.data;
            const statusColor =
              ENROLLMENT_STATUS_COLORS[d.status ?? 'not_started'] ?? 'bg-gray-100 text-gray-500';
            return (
...
export function LMSPageClient({
  ...
  const totalEnrollments =
    summary.totalEnrollments ?? courses.reduce((sum, c) => sum + (c.data.enrollments ?? 0), 0);
  ...
  const computedCompletionRate =
    courses.length > 0
      ? Math.round(
          ...
          }, 0) / (courses.filter((c) => (c.data.enrollments ?? 0) > 0).length || 1),
        )
      : 0;
  ...
  return (
    <div className="px-8 py-8 max-w-7xl">
      ...
      {activeTab === 'enrollments' && <EnrollmentsTable enrollments={enrollments} />}
