...
import { Plus, X, Users, MapPin, CheckCircle, Clock, Wrench } from 'lucide-react';
...
function RequestsTab({ requests, onNewRequest, onUpdate }: RequestsTabProps) {
  ...
  const completedToday = requests.filter(
    (r) => r.status === 'completed' && r.completedAt?.slice(0, 10) === today,
  ).length;
              ...
              <tbody>
                {filtered.map((req) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="text-base" title={req.type}>
                        {REQUEST_TYPE_ICONS[req.type ?? 'other']}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium capitalize ${PRIORITY_BADGE[req.priority ?? 'medium'] ?? PRIORITY_BADGE.medium}`}
                      >
                        {req.priority ?? 'medium'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_BADGE[req.status ?? 'open'] ?? STATUS_BADGE.open}`}
                      >
                        {req.status ?? 'open'}
                      </span>
                    </td>
      ...
      {summary.avgCompletionHours != null && (
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <p className="text-3xl font-semibold text-gray-900">
            {summary.avgCompletionHours.toFixed(1)} hrs
          </p>
        </div>
      )}
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [reqRes, sumRes] = await Promise.all([
        fetch(`${API_BASE}/facility-requests`, {
          headers: { 'x-tenant-id': process.env.NEXT_PUBLIC_TENANT_ID ?? 'demo-tenant' },
        }),
        fetch(`${API_BASE}/facility-requests/summary`, {
          headers: { 'x-tenant-id': process.env.NEXT_PUBLIC_TENANT_ID ?? 'demo-tenant' },
        }),
      ]);
      if (reqRes.ok) {
