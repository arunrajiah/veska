...
export function PayslipsClient({ payslips }: { payslips: PayrollItem[] }) {
  ...
  const periods = Array.from(
    new Set(payslips.map((p) => p.data?.period).filter(Boolean) as string[]),
  ).sort();

  const filtered =
    periodFilter === 'all' ? payslips : payslips.filter((p) => p.data?.period === periodFilter);
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((slip) => {
                ...
                const totalDeductions = (sd?.deductions ?? []).reduce(
                  (s, d) => s + (d.amount ?? 0),
                  0,
                );
                const hasBreakdown =
                  (sd?.deductions?.length ?? 0) > 0 || (sd?.allowances?.length ?? 0) > 0;
                  ...
                  <>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-600">
                        {totalDeductions > 0 ? usd(totalDeductions) : '—'}
                      </td>
