...
export default async function CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [contacts, deals] = await Promise.all([
    apiFetch<{ data: ContactRecord[] } | ContactRecord[]>(
      `/api/v1/crm/contacts?companyId=${id}&limit=20`,
      TENANT_ID,
    )
      .then((res) => (Array.isArray(res) ? res : ((res as { data: ContactRecord[] }).data ?? [])))
      .catch(() => [] as ContactRecord[]),
    apiFetch<{ data: DealRecord[] } | DealRecord[]>(
      `/api/v1/crm/deals?companyId=${id}&limit=20`,
      TENANT_ID,
    )
