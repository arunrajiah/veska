...
export default async function ContactDetailPage({ params }: { params: { id: string } }) {
  ...
  try {
    record = await apiFetch<ContactRecord>(`/api/v1/entities/Contact/${id}`, tenantId);
  } catch {
    record = null;
  }
  ...
  const d = record.data;
  const fullName = [d['first_name'], d['last_name']].filter(Boolean).join(' ') || 'Contact';
  ...
  return (
    <div className="px-8 py-8 max-w-3xl">
      <div className="mb-6">
        <Link href="/dashboard/crm/contacts" className="text-xs text-gray-400 hover:text-gray-700">
          ← Contacts
        </Link>
      </div>
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <div className="flex gap-2">
          <EnrichButton contactId={id} tenantId={tenantId} />
          <Link
