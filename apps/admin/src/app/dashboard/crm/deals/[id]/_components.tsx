...
export function EditDealSlideOver({ dealId, initial, onClose, onSaved }: EditDealProps) {
  ...
  const [contactId, setContactId] = useState(
    String(initial['contactId'] ?? initial['contact_id'] ?? ''),
  );
  ...
  const [stage, setStage] = useState(
    String(initial['stage'] ?? initial['stageId'] ?? 'prospecting'),
  );
  const [closeDate, setCloseDate] = useState(
    String(initial['close_date'] ?? initial['closeDate'] ?? '').slice(0, 10),
  );
      ...
      {allStages.map((stage, idx) => {
        ...
        return (
          <div key={stage} className="flex items-center gap-3">
            <div
              className={`w-2 h-2 rounded-full flex-shrink-0 ${isCurrent ? 'bg-indigo-600' : isPast ? 'bg-gray-300' : 'bg-gray-100 border border-gray-200'}`}
            />
            <span
              className={`text-xs ${isCurrent ? 'font-medium text-gray-900' : isPast ? 'text-gray-400 line-through' : 'text-gray-400'}`}
            >
...
export function DealDetailClient({ dealId, data: d, stage, aiSummary }: DealDetailClientProps) {
  ...
  const fields = [
    ...
    {
      label: 'Close Date',
      value: String(d['close_date'] ?? d['closeDate'] ?? '').slice(0, 10) || '',
    },
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            {stage && (
              <span
                className={`text-xs px-2 py-0.5 rounded-full font-medium ${stageInfo ?? 'bg-gray-100 text-gray-600'}`}
              >
                {STAGE_LABEL[stage] ?? stage}
              </span>
            )}
