      ...
      <div className="relative bg-white w-full max-w-md h-full overflow-y-auto shadow-xl flex flex-col">
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="flex-1 px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input name="headCount" type="number" min="0" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gray-900" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Manager ID</label>
            <input name="managerId" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gray-900" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Budget (USD)</label>
            <input name="budget" type="number" min="0" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gray-900" />
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Department Name</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Head Count</th>
              ...
              {initial.map((dept) => {
                ...
                return (
                  <tr key={dept.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-gray-500">{d.budget != null ? formatCurrency(d.budget) : '—'}</td>
                  </tr>
                );
              })}
            </tbody>
