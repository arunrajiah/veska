...
export default async function HRDashboard() {
  ...
  const pendingLeave = leaveRecords.filter(
    (r) => (r.data.status ?? 'pending') === 'pending',
  ).length;
  const recentEmployees = [...employees]
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
    .slice(0, 5);
  const pendingLeaveRecords = leaveRecords
    .filter((r) => (r.data.status ?? 'pending') === 'pending')
    .slice(0, 5);
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right">
                          <StatusBadge status={emp.data.status} />
                        </td>
                      </tr>
                    );
                  })}
                      ...
                      >
                        <td className="px-4 py-3">
                          ...
                          <p className="text-xs text-gray-500 capitalize">
                            {type} · {startDate ? startDate.slice(0, 10) : '—'}
                          </p>
                        </td>
                        <td className="px-4 py-3 text-right">
