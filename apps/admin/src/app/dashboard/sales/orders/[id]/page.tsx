...
export default async function SalesOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  try {
    record = await apiFetch<OrderRecord>(`/api/v1/sales/orders/${params.id}`, tenantId);
  } catch {
    record = null;
  }
  ...
  if (!record) {
    return (
      <div className="px-8 py-8">
        ...
        <Link href="/dashboard/sales/orders" className="text-sm text-gray-600 hover:text-gray-900 mt-2 inline-block">
          ← Back to orders
        </Link>
      </div>
    );
      ...
      <div className="flex items-start gap-6">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-6">
            <h1 className="text-2xl font-semibold text-gray-900 font-mono">{soNumber}</h1>
            <span
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
            <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm mb-6">
              <div>
                ...
                <p className="text-gray-800">{String(d['customer_name'] ?? '—')}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Expected delivery</p>
                <p className="text-gray-800">{String(d['delivery_date'] ?? '—')}</p>
              </div>
              {d['notes'] && (
