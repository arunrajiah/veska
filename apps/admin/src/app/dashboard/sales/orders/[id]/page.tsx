...
export default async function SalesOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!record) {
    return (
      <div className="px-8 py-8">
        ...
        <Link href="/dashboard/sales/orders" className="mt-4 inline-block text-sm text-gray-600 hover:text-gray-900">← Back to orders</Link>
      </div>
    );
  }
  ...
  const items = d.items ?? [];
  const subtotal = d.subtotal ?? items.reduce((sum, item) => sum + ((item.quantity ?? 0) * (item.unitPrice ?? 0)), 0);
  const tax = d.tax ?? 0;
  ...
  return (
    <div className="px-8 py-8 max-w-5xl">
      <div className="mb-4">
        <Link href="/dashboard/sales/orders" className="text-xs text-gray-400 hover:text-gray-700">← Sales Orders</Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-5">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
            <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm mb-6">
              <div>
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Customer</p>
              ...
              <div>
                ...
                <p className="text-gray-800">{d.deliveryDate ? d.deliveryDate.slice(0, 10) : '—'}</p>
              </div>
              {d.shippingAddress && (
                <div>
                  <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Shipping Address</p>
                  ...
                  <table className="w-full text-sm">
                      ...
                      {items.map((item, i) => {
                        ...
                        return (
                          <tr key={i} className="border-b border-gray-50 last:border-0">
                            <td className="px-4 py-2.5 text-gray-700">{item.productName || item.productId || `Item ${i + 1}`}</td>
                            <td className="px-4 py-2.5 text-right text-gray-600">{item.quantity ?? 0}</td>
                            <td className="px-4 py-2.5 text-right text-gray-600">{formatCurrency(item.unitPrice ?? 0)}</td>
                            <td className="px-4 py-2.5 text-right font-medium text-gray-800">{formatCurrency(lineTotal)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                    <tfoot>
                      <tr className="border-t border-gray-100 bg-gray-50/50">
                        ...
                        <td className="px-4 py-2 text-right text-gray-700">{formatCurrency(subtotal)}</td>
                      </tr>
                      <tr className="bg-gray-50/50">
                        <td colSpan={3} className="px-4 py-2 text-xs text-gray-500 text-right">Tax</td>
                        <td className="px-4 py-2 text-right text-gray-700">{formatCurrency(tax)}</td>
                      </tr>
                      <tr className="border-t border-gray-200 bg-gray-50">
                        <td colSpan={3} className="px-4 py-2.5 text-sm font-semibold text-gray-700 text-right">Grand Total</td>
                        <td className="px-4 py-2.5 text-right text-sm font-bold text-gray-900">{formatCurrency(total)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4">
            <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-3">Order Summary</p>
