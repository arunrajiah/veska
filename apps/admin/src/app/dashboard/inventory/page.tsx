...
export default async function InventoryDashboard() {
  ...
  const lowStockItems = products.filter((p) => (p.data.stockQuantity ?? 0) <= (p.data.reorderLevel ?? 0));
  ...
  return (
    <div className="px-8 py-8 max-w-6xl">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Products', value: totalProducts, href: '/dashboard/inventory/products' as Route },
          { label: 'Low Stock Items', value: lowStockItems.length, href: '/dashboard/inventory/stock' as Route },
          { label: 'Inventory Value', value: formatCurrency(totalValue), href: '/dashboard/inventory/products' as Route },
          { label: 'Active Warehouses', value: activeWarehouses, href: '/dashboard/inventory/warehouses' as Route },
        ].map((s) => (
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Low Stock Alerts</h2>
            <Link href="/dashboard/inventory/stock" className="text-xs text-indigo-600 hover:text-indigo-800">View all →</Link>
          </div>
            ...
            <table className="w-full text-sm">
              <tbody>
                {lowStockItems.slice(0, 8).map((prod) => {
                  ...
                  return (
                    <tr key={prod.id} className="border-b border-gray-50 last:border-0 bg-red-50/50 hover:bg-red-50">
                      <td className="px-4 py-3">
                        <Link href={`/dashboard/inventory/products/${prod.id}`} className="font-medium text-gray-900 hover:underline">
                          {prod.data.name || '—'}
                        </Link>
                        {prod.data.sku && <p className="text-xs text-gray-400 font-mono">{prod.data.sku}</p>}
                      </td>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            ...
            <Link href="/dashboard/inventory/movements" className="text-xs text-indigo-600 hover:text-indigo-800">View all →</Link>
          </div>
                ...
                {recentMovements.map((mv) => {
                  ...
                  return (
                    <tr key={mv.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900">{d.productName || d.productId || '—'}</p>
