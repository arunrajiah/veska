...
/**
 * Resolve a WhatsApp phone number to a known Identity for the given tenant.
 *
 * The phoneNumber is the E.164 number stripped of the 'whatsapp:' prefix
 * (e.g. '+14155238886').
 *
 * Resolution order:
 * 1. Existing identity with channelIds->>'whatsapp' = phoneNumber
 * 2. Contact entity with data->>'phone' = phoneNumber → create contact identity
...
export async function resolveWhatsAppIdentity(
  ...
  // 1. Check for an existing identity with this WhatsApp channel ID
  const existing = await db.query.identities.findFirst({
    where: and(
      eq(schema.identities.tenantId, tenantId),
      sql`${schema.identities.channelIds}->>'whatsapp' = ${phoneNumber}`,
    ),
  });
  ...
  // 2. Check if a Contact entity record exists with this phone number
  const contact = await db.query.entityRecords.findFirst({
    where: and(
      ...
      sql`${schema.entityRecords.data}->>'phone' = ${phoneNumber}`,
    ),
  });
  ...
  if (contact) {
    const [created] = await db
      ...
      .values({
        tenantId,
        type: 'contact',
        channelIds: { whatsapp: phoneNumber },
        roleIds: ['viewer'],
